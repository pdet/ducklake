    ...
    WHERE table_id=%d
		...
		for (auto &chunk : entry.data->data->Chunks()) {
			for (idx_t r = 0; r < chunk.size(); r++) {
				...
				values += "(";
				values += to_string(entry.data->GetRowId(global_row_idx, NumericCast<int64_t>(entry.row_id_start)));
				global_row_idx++;
...
string DuckLakeMetadataManager::WriteNewInlinedDeletes(const vector<DuckLakeDeletedInlinedDataInfo> &new_deletes) {
	...
	for (auto &entry : new_deletes) {
		...
		// overwrite the snapshot for the old rows (exclude newly inserted rows in this same snapshot)
