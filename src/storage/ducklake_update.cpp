...
OperatorResultType DuckLakeUpdate::Execute(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
	...
	const idx_t expression_count = update_expression_chunk.ColumnCount();

	// build output, all expression results (physical columns + partition transforms) + row_id
	for (idx_t i = 0; i < expression_count; i++) {
		insert_chunk.data[i].Reference(update_expression_chunk.data[i]);
	}
	// we place row_id right after all expressions
	insert_chunk.data[expression_count].Reference(input.data[row_id_index]);
